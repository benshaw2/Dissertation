...
clean_val_loader = DataLoader(clean_val_dataset, batch_size=64, shuffle=False, num_workers=4)
blurred_val_loader = DataLoader(blurred_val_dataset, batch_size=64, shuffle=False, num_workers=4)
...
def train_with_symmetry(model, trainloader, VF, symmetry_loss_weight=0.5, optimizer_class=torch.optim.Adam,
                        ...
                        lr=1e-4, n_epochs=5, sym_mult=1.0):
...
for i in range(1):
    ...
    result = train_with_symmetry(
        ...
        lr=0.0,
        nosymtime=1.0,
        sym_mult=(3 * 224 * 224) ** 2  # You can adjust this multiplier
    )
