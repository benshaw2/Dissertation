...
target_transform_one_hot_f = transforms.Lambda(lambda y: F.one_hot(torch.tensor(y), num_classes=43).float())
...
optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-4)  # Equivalent to l2_lambda = 0.0001
  # Adam optimizer'''
...
for i in range (1): #range(10):
	...
	with open('gamma_gtsrb/plsym_losses' + str(i) + '.csv', 'w', newline='') as file:
	    writer = csv.writer(file)
	    for row in result[1]:
		writer.writerow([row])

	with open('gamma_gtsrb/plsym_times' + str(i) + '.csv', 'w', newline='') as file:
	    writer = csv.writer(file)
	    for row in result[2]:
		writer.writerow([row])
	...
	df.to_csv("gamma_gtsrb/plsym_accs' + str(i) + '.csv")

	torch.save(model.state_dict(), "gamma_gtsrb/plsym_model" + str(i))
